...
def parse_args():
	parser = argparse.ArgumentParser(description="Script 1c computing ROC AUC and simple statistics on selected set of results from DeePromoter models results (output of script_1a.py with flag -p)",
...
* 1 - promoter""",default=0,choices = [0,1],type=int)
	parser.add_argument('-o','--out', nargs=1, help="""Name of output .txt file, where results from Fisher test will be saved;
Default: results/DeePromoter_out_AUC.txt""",default=None)
	...
	if args.out is None:
		if not os.path.isdir("results"):
			os.mkdir("results")
		out=f'results/DeePromoter_out_AUC.txt'
...
def main():
	...
	if not inputs is None:
		with open(inputs[1],"w") as out:
			...
			AUC_values=[[] for x in range(len(L_DEEP))]
			AUC_class=[[] for x in range(len(L_DEEP))]
#			print(AUC_class)
			for i in range(len(inputs[0])):
				...
				for j in range(len(L_DEEP)):
					...
					AUC_values[j].extend(file.iloc[:,j].values)
					AUC_class[j].extend([c for x in range(file.shape[0])])
#			print(len(AUC_values[0]))
#			print(AUC_values[0][:10])
#			print(AUC_values[1][:10])
#			print(AUC_values[2][:10])
#			print(AUC_values[3][:10])
			for i in range(len(L_DEEP)):
#				print(np.array_equal(np.array(AUC_values[0]),np.array(AUC_values[i])))
#				print(sum(AUC_class[i]))
#				print(sum(AUC_values[i]))
				...
				if sum(contingency_table[i,0,:]) and sum(contingency_table[i,1,:]):
					AUC=roc_auc_score(AUC_class[i],AUC_values[i])
				else:
					AUC="NaN"
